...
const FILE_MAP = [
  ...
  { path: 'pnpm-workspace.yaml', source: 'template:root/pnpm-workspace.yaml.hbs' },
...
function renderFile(pathName, inputs) {
  ...
  if (pathName === 'README.md') {
    return [
      `# ${name}`,
      '',
      'Generated by fullstack-v1 write mode (Step 5 web scaffold integration).',
      '',
      `- withAuth: ${withAuth}`,
      `- database: ${database}`,
      '',
  ...
  if (pathName === 'pnpm-workspace.yaml') {
    return [
      'packages:',
      '  - "apps/*"',
      '  - "packages/*"',
  ...
  if (pathName === 'apps/api/src/main.ts') {
    return [
      ...
      '',
      'server.listen(port, () => {',
      `  console.log(\`[fullstack-v1] api ready: ${name} (${authLabel}, ${database}) on http://localhost:\${port}\`);`,
      '});',
      '',
    ].join('\n');
  }
  ...
  if (pathName === 'apps/web/src/app/layout.tsx') {
    return [
      ...
      `  title: 'fullstack-v1 ${name}',`,
      "  description: 'Generated by fullstack-v1.',",
      '};',
      '',
  ...
  if (pathName === 'apps/web/src/app/page.tsx') {
    return [
      ...
      '  return (',
      '    <main>',
      `      <h1>fullstack-v1 ${name}</h1>`,
      '      <p>This page was generated by the fullstack-v1 template.</p>',
      '    </main>',
      '  );',
      '}',
      '',
