...
const TABS = ['Trending','New','Most Remixed','Most Viewed','Staff Picks','AI Companies'];
...
function applySortAndFilter(tab: string, category: string, search: string) {
  let apps = [...SEEDED_APPS];

  if (tab === 'New') {
    apps = apps.filter(a => !a.guarded);
    apps = apps.reverse();
  } else if (tab === 'Most Remixed') {
    apps = apps.sort((a, b) => b.remixes - a.remixes);
  } else if (tab === 'Most Viewed') {
    apps = apps.sort((a, b) => b.views - a.views);
  } else if (tab === 'Staff Picks') {
    apps = apps.filter(a => STAFF_PICK_IDS.has(a.id));
  } else if (tab === 'AI Companies') {
    apps = apps.filter(a => AI_COMPANY_IDS.has(a.id));
  }

  if (category !== 'All') {
    const cat = category.toLowerCase().replace('-', '');
    apps = apps.filter(a => a.category.replace('-', '') === cat || a.category === category.toLowerCase());
  }

  if (search.trim()) {
    const q = search.toLowerCase();
    apps = apps.filter(a => a.name.toLowerCase().includes(q) || a.creator.toLowerCase().includes(q));
  }
...
function StatCounter({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '0 24px' }}>
      <div style={{ fontSize: 28, fontWeight: 700, color: TEXT, fontVariantNumeric: 'tabular-nums' }}>
        {v.toLocaleString()}
      </div>
...
function LiveCounter() {
  ...
  useEffect(() => {
    const t = setInterval(() => setCount(c => c + (Math.random() > 0.5 ? 1 : -1) > 0 ? c + (Math.random() > 0.5 ? 1 : 0) : c), 5000);
    return () => clearInterval(t);
  }, []);
  return (
    <div style={{ textAlign: 'center', padding: '0 24px' }}>
      <div style={{ fontSize: 28, fontWeight: 700, color: ACCENT_GRN, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        <span style={{ width: 8, height: 8, borderRadius: '50%', background: ACCENT_GRN, display: 'inline-block', boxShadow: `0 0 8px ${ACCENT_GRN}` }} />
        {count}
      </div>
      <div style={{ fontSize: 12, color: TEXT_MUTED, marginTop: 2 }}>apps live right now</div>
    </div>
  );
}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
          <span style={{
            ...
            background: app.live ? ACCENT_GRN : (app.live === false ? TEXT_DIM : '#f59e0b'),
  ...
  return (
    <div style={{ ...GLASS, padding: 20, flex: '1 1 280px', minWidth: 260 }}>
      ...
      <div style={{ padding: '10px 14px', borderRadius: 10, background: `rgba(99,102,241,0.12)`, border: `1px solid rgba(99,102,241,0.25)`, marginBottom: 8 }}>
        <div style={{ fontSize: 18 }}>{chain.root.emoji} <span style={{ fontSize: 14, fontWeight: 700, color: TEXT }}>{chain.root.name}</span></div>
        <div style={{ fontSize: 11, color: TEXT_MUTED }}>@{chain.root.creator}</div>
      </div>
      {chain.children.map((child, i) => (
        <div key={i}>
          ...
          <div style={{ padding: '8px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: `1px solid ${BORDER}`, marginLeft: child.depth * 20, marginBottom: 6 }}>
            <div style={{ fontSize: 14 }}>{child.emoji} <span style={{ fontSize: 13, color: TEXT }}>{child.name}</span></div>
            <div style={{ fontSize: 11, color: TEXT_MUTED }}>@{child.creator}</div>
          </div>
        </div>
      ))}
    </div>
...
export default function DiscoverPage() {
  ...
  const filtered = applySortAndFilter(activeTab, activeCategory, search);
  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', color: TEXT, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 24px 60px' }}>
        {/* Hero stats strip */}
        <div style={{ ...GLASS, display: 'flex', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', padding: '24px 0', marginBottom: 40, gap: 0 }}>
          <StatCounter value={12847} label="apps built" />
          <div style={{ width: 1, height: 40, background: BORDER }} />
          <StatCounter value={4291} label="creators" />
          <div style={{ width: 1, height: 40, background: BORDER }} />
          <StatCounter value={89342} label="remixes" />
          <div style={{ width: 1, height: 40, background: BORDER }} />
          <LiveCounter />
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 15, color: TEXT_MUTED, marginTop: 6 }}>Explore apps built by the Factory community. Remix anything.</p>
        </div>
        ...
        <input
          ...
          style={{
            width: '100%', boxSizing: 'border-box', padding: '10px 16px', borderRadius: 10, border: `1px solid ${BORDER}`,
            background: 'rgba(255,255,255,0.04)', color: TEXT, fontSize: 14, outline: 'none', marginBottom: 14,
          }}
        />
